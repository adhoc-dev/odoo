...
class l10nLatamAccountPaymentCheck(models.Model):
    ...
    def _auto_init(self):
        ...
        if not index_exists(self.env.cr, 'l10n_latam_check_unique'):
            # issue_state is used to know that is an own check and also that is paid
            self.env.cr.execute("""
    ...
    def _get_last_operation(self):
        ...
        return (self.payment_id + self.operation_ids).filtered(
                lambda x: x.state in ['in_process','paid']).sorted(key=lambda payment: (payment.date, payment._origin.id))[-1:]
    ...
    def button_open_check_operations(self):
        ...
        '''
        self.ensure_one()
        operations = ((self.operation_ids + self.payment_id).filtered(lambda x: x.state in ['in_process', 'paid']))
        action = {
    ...
    def _unlink_if_payment_is_draft(self):
        if any(check.payment_id.state in ['in_process', 'paid'] for check in self):
