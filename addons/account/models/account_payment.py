...
class AccountPayment(models.Model):
    ...
    journal_id = fields.Many2one(
        ...
        help="Only journals with an outstanding payment account are visible"
    )
    ...
    def _compute_company_id(self):
        for payment in self:
            if payment.journal_id.company_id:
                payment.company_id = payment.journal_id.company_id
            else:
    ...
    def _compute_available_journal_ids(self):
        ...
        for pay in self:
            ...
            ).filtered(lambda j: j._is_payment_method_available(pay.payment_method_id.code))
    ...
    def _compute_outstanding_account_id(self):
        for pay in self:
            if pay.journal_id:
                pay.outstanding_account_id = pay.journal_id.outstanding_payment_account_id
            else:
