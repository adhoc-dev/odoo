...
class TestManual(common.TestAr):
    ...
    def test_20_corner_cases(self):
        ...
        cases = {'test_invoice_1': '"Mono" partner of tipe Service and VAT 21',
                 ...
                 'test_invoice_11': '"RI" partner with many lines in order to prove rounding error, with 4'
                 ' decimals of precision for the currency and 2 decimals for the product the error apperar',
                 'test_invoice_12': '"RI" partner with many lines in order to test rounding error, it is required'
                 ' to use a 4 decimal precision in prodct in order to the error occur',
                 'test_invoice_13': '"RI" partner with many lines in order to test zero amount'
                 ...
                 'test_invoice_17': '"RI" partner with 100%% of discount',
                 'test_invoice_18': '"RI" partner with 100%% of discount and with different VAT aliquots'}
        self._test_demo_cases(cases)

    def test_21_currency(self):
        self._prepare_multicurrency_values()
        self._test_demo_cases({'test_invoice_10': '"Responsable Inscripto" in USD and VAT 21'})
