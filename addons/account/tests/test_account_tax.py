...
class TestAccountTax(AccountTestInvoicingCommon):
    ...
    def test_logging_of_repartition_lines_update_when_tax_is_used(self):
        ...
        last_invoice_rep_line = self.company_data["default_tax_sale"].invoice_repartition_line_ids.filtered(
            lambda tax_rep: not tax_rep.factor_percent
        )
        last_refund_rep_line = self.company_data["default_tax_sale"].refund_repartition_line_ids.filtered(
            lambda tax_rep: not tax_rep.factor_percent
        )
        ...
        self.company_data["default_tax_sale"].write(
            {
                "invoice_repartition_line_ids": [
                    Command.update(
                        ...
                        {"factor_percent": -100, "tag_ids": [Command.create({"name": "TaxTag12345"})]},
                    ),
                ],
                "refund_repartition_line_ids": [
                    Command.update(
        ...
        previews = self.company_data["default_tax_sale"].message_ids.mapped("preview")
        self.assertIn(
            "Invoice repartition line 3: 0.0 -100.0 (Factor Percent) None ['TaxTag12345'] (Tax Grids)", previews
        )
        self.assertIn(
    ...
    def test_logging_of_repartition_lines_removal_when_tax_is_used(self):
        ...
        last_invoice_rep_line = self.company_data["default_tax_sale"].invoice_repartition_line_ids.sorted(
            key=lambda r: r.sequence
        )[-1]
        last_refund_rep_line = self.company_data["default_tax_sale"].refund_repartition_line_ids.sorted(
            key=lambda r: r.sequence
        )[-1]
    ...
    def test_round_globally_zero_percent_tax(self):
        ...
        debit_before = sum(line.debit for line in move.line_ids)

        # Add 0% tax to line
        move.invoice_line_ids[0].tax_ids = [(6, 0, [tax_0.id])]
        move._recompute_dynamic_lines()
        debit_after = sum(line.debit for line in move.line_ids)

        self.assertEqual(debit_before, debit_after, "Debit/Credit should not change when applying a 0% tax.")
