...
class L10nARWebsiteSale(WebsiteSale):
    ...
    def _get_render_values(self, kw, values):
        render_values = super()._get_render_values(kw, values)
        if request.website.sudo().company_id.country_id.code == "AR":
            render_values.update({'identification_types': request.env['l10n_latam.identification.type'].search(['|', ('country_id', '=', False), ('country_id.code', '=', 'AR')]),
